...
/*
There are altogether m*n points.
All points in the same land are in the same tree, all point to the one single root.
We need O(logmn) to visit the root from any point in the tree, so O(k * logmn) in all.
 */
...
public class Solution {
    ...
    private int[] findRoot(int[] point, int[][][] roots) {
        int[] root = roots[point[0]][point[1]];
        while (!(point[0] == root[0] && point[1] == root[1])) {
            point = root;
            root = roots[point[0]][point[1]];
        }
        return root;
    }
    ...
    public List<Integer> numIslands2(int m, int n, int[][] positions) {
        ...
        for (int[] point: positions) {
            ...
            Set<int[]> connLands = new HashSet<>();  // This has to be Set, don't add same land multiple times.
            for (int d = 0; d < 4; d ++) {
                int x = point[0] + D[d][0];
                int y = point[1] + D[d][1];
                if (x >= 0 && x < m && y >= 0 && y < n && matrix[x][y] == 1)
                    connLands.add(findRoot(roots[x][y], roots));
            }

            int count = (res.isEmpty() ? 0 :res.get(res.size() - 1)) + 1;
            roots[point[0]][point[1]] = point;  // first make the point as a new land
            for (int[] land: connLands) {
                int[] root = findRoot(point, roots);
                if (!(root[0] == land[0] && root[1] == land[1])) {  // they are not the same land
                    roots[root[0]][root[1]] = land;  // merge land
                    count --;
                }
            }
            res.add(count);
